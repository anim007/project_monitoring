...
class ActionColumn extends yii\grid\Column
{
    ...
    {
        $this->initDefaultButton('view', 'eye', [], 'secondary');
        $this->initDefaultButton('update', 'pencil-alt', [], 'primary');
     ...
     * @since 2.0.11
     */
    protected function initDefaultButton($name, $iconName, $additionalOptions = [], $classButton = null)
    {
        if (!isset($this->buttons[$name]) && strpos($this->template, '{' . $name . '}') !== false) {
            $this->buttons[$name] = function ($url, $model, $key) use ($name, $iconName, $additionalOptions, $classButton) {
                switch ($name) {
                    case 'view':
                ...
                if ($iconName == 'trash-alt') {
                    $icon = '<i class="far fa-' . $iconName . '"></i>';
                } else {
                    $icon = '<i class="fas fa-' . $iconName . '"></i>';
                }
                $class = !is_null($classButton) ? $classButton : 'default';
                $button = '<button class="btn btn-sm btn-' . $class . '">' . $icon . '</button>';
